"""
Comprehensive Seizure Dataset Validation
Checks all aspects of seizure data quality
...
print("\n" + "="*70)
print("SEIZURE DATASET VALIDATION - COMPREHENSIVE CHECK")
print("="*70)
...
OUTPUT_DIR = Path('seizure_detection/data_validation')
            ...
            try:
                ...
                if not (0 <= x_center <= 1 and 0 <= y_center <= 1 and 
                    ...
                    invalid_labels.append({
                        'file': stem,
                        'line': line_num,
                        'error': f'Coordinates out of range [0,1]'
                    })
                    continue
...
if total_boxes > 0:
    imbalance_ratio = max(seizure_count, normal_count) / min(seizure_count, normal_count) if min(seizure_count, normal_count) > 0 else float('inf')
    print(f"\n📊 Class Balance:")
    print(f"   Ratio (seizure:normal) = {seizure_count}:{normal_count}")
...
sample_size = min(len(paired_stems), 500)
...
for stem in random.sample(paired_stems, min(100, len(paired_stems))):
    label_path = TEST_LABELS_DIR / f"{stem}.txt"
    with open(label_path) as f:
        class_id = int(f.readline().split()[0])
        if class_id == 1 and len(seizure_samples) < 6:
            seizure_samples.append(stem)
        elif class_id == 0 and len(normal_samples) < 6:
            normal_samples.append(stem)
    if len(seizure_samples) >= 6 and len(normal_samples) >= 6:
        break

samples = seizure_samples[:6] + normal_samples[:6]
...
plt.suptitle('Sample Images with Labels (Red=Seizure, Blue=Normal)', 
...
if total_boxes > 0:
    ...
    axes[0].set_title('Class Distribution in Seizure Test Set', fontsize=14, fontweight='bold')
...
for i, count in enumerate([class_counts[j] for j in sorted(CLASS_NAMES.keys())]):
...
report = {
    ...
    'class_balance': {
        ...
        'ratio': f"{seizure_count}:{normal_count}",
...
print(f"   Seizure boxes: {seizure_count} ({seizure_count/total_boxes*100:.1f}%)")
